...
def prepare_execution_data(datas):
    ...
    print(
        f"    代码覆盖率: {valid_code_count / total_computations * 100:.1f}%" if total_computations > 0 else "    代码覆盖率: 0%")
    ...
    for in_file in tqdm(in_files, desc="处理文件"):
        ...
        try:
            ...
            for idx in tqdm(range(0, len(execution_tasks), batch_size), desc="执行批次"):
                ...
                try:
                    print(
                        f"    执行批次 {idx // batch_size + 1}/{(len(execution_tasks) + batch_size - 1) // batch_size}, 代码数量: {len(batch_code)}")
                    ...
                    for result, task in zip(batch_result, batch_tasks):
                        ...
                        error_info = result[1].get("concise_exec_info", "").strip() if isinstance(result[1],
                                                                                                  dict) else str(
                            result[1]).strip()
